...
def _notify_recurring_failed(recurring: RecurringOrder, error: str) -> None:
    ...
    if customer.email:
        try:
            send_email(
                to=[customer.email],
                subject=_("Your recurring order '%(label)s' could not be refilled") % {"label": recurring.label},
                text_body=_(
                    "Hi,\n\nWe couldn't generate your next refill for '%(label)s': %(error)s\n"
                    "We'll try again on the next cycle - no action is needed unless the problem persists.\n"
                )
...
def run_due_recurring_orders(*, now=None, lead_time_hours: int = 24) -> dict:
    ...
    for recurring in RecurringOrder.objects.filter(is_active=True, next_run_at__lte=now + timedelta(hours=lead_time_hours)).select_related("address", "customer"):
        ...
        try:
            ...
            _notify_recurring_success(recurring, order)
