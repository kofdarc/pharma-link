...
class Prescription(UUIDTimeStampedModel):
    """
    A prescription is consumable by ANY pharmacy, including pharmacies with no HealthConnect account.
    Security model:
...
class PrescriptionItem(UUIDTimeStampedModel):
    ...
    # Catalog link is optional so a doctor can prescribe something not yet in the HealthConnect catalog.
...
class PrescriptionDispense(UUIDTimeStampedModel):
    """One consumption event. `pharmacy` is null for pharmacies that have no HealthConnect account."""
