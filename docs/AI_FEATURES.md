...
|---|---|---|---|
| Brand↔generic alias search ("panadol" → paracetamol) | ✅ Built · deterministic | Already worked **if** the alias exists as a row; every catalog item's generic name is auto-aliased at seed time (`seed_poc.py`). | Coverage is still only as good as the seeded/imported alias set — real onboarding data will need the same treatment. |
...
| Arabic-script alias seeding for the demo catalog | ✅ Built · deterministic | `MedicineAlias.AliasType.TRANSLITERATION` (new choice, migration `0005`) + seeded aliases for the 8 highest-traffic demo brands in `seed_poc.py` (e.g. Panadol → `بانادول`). | This is the "alias-coverage data pass" — a data task, not new matching logic. Exact and fuzzy search both pick these up for free through the existing `icontains`/`SequenceMatcher` paths. |
