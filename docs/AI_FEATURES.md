...
| Handwriting transcription via a **non-Anthropic** vision model | ✅ Built · optional (`PRESCRIPTION_OCR_PROVIDER=openai_vision`) | `OpenAiVisionOcrProvider` — the same transcribe-only prompt and guardrails as `AnthropicOcrProvider`, but over any OpenAI-compatible `/chat/completions` endpoint that accepts image content blocks: OpenRouter (`google/gemini-2.5-flash`, `qwen/qwen-2.5-vl-72b-instruct`, ...), a local Ollama (`llama3.2-vision`, `qwen2-vl` — fully offline, no image egress), Groq, OpenCode Zen. Reason it exists: patient uploads are overwhelmingly **handwritten**, and classical OCR (Tesseract/EasyOCR) only reads print — a vision model is the floor for this use case, and this provider gets there without tying the platform to one vendor. Images render page-by-page for PDFs via `pdf2image` (same as Tesseract). Configured by `PRESCRIPTION_OCR_VISION_BASE_URL` / `_API_KEY` / `_MODEL` (`_MODEL` has no default — name a vision-capable model); inert until all three are set. Same data-handling caveat as the Anthropic row for any *hosted* gateway — an image can carry patient/doctor names. |
| **Single-call image → structured fields** (best available read on handwriting) | ✅ Built · optional (`PRESCRIPTION_OCR_PROVIDER=vision_structured` or `anthropic_structured`) | `VisionStructuredOcrProvider` (any OpenAI-compatible vision gateway, reuses `PRESCRIPTION_OCR_VISION_*`) and `AnthropicStructuredOcrProvider` (Claude, reuses `ANTHROPIC_API_KEY` / `ANTHROPIC_OCR_MODEL`) go from image **straight to structured fields in one call**, instead of transcribing to plain text and re-parsing that text with `PRESCRIPTION_NLP_PROVIDER`. They set `OcrProvider.supports_structured`, which is how `ocr_and_structure` knows to skip the NLP stage; if the call fails it falls back to the two-stage path so an upload is never lost. `extract_text()` still works — it renders the same single read as plain text, so the pharmacist 'extract candidate lines' action and cached `ocr_text` are unchanged. | **Why the split was the problem:** the transcribe-only stage is prompted to emit flat text, and by the time that text reaches the regex/NLP extractor the page context that made a scrawl legible — layout, dose columns, a brace tying "after meals" to two drugs — is gone. Found in production on a handwritten Indian dental script (Augmentin 625mg, Enzoflam, Pan-D 40mg, Hexigel; `1-0-1` dose columns; an unlabelled patient line): the regex path dropped the two rows with no printed strength outright, matched none of the four against the Lebanese catalog, parsed no header field, and scored **0.0** — so the patient was shown "we couldn't read this prescription clearly enough" over a page that was perfectly legible. Two fixes, both needed: this provider pair, and the confidence rework in the row below. Dosing shorthand is captured twice on purpose — `dose_pattern` holds the notation verbatim ("1-0-1"), `directions` holds the plain reading of it — so a wrong expansion can never silently replace what the prescriber actually wrote. Same boundary as every other provider (docs/PRD.md non-goals): transcribe and lay out, never diagnose, advise, or substitute; output keys are allowlisted and length-bounded, and the image is data, not instructions. Same data-handling caveat — the image leaves the box. |
| Structured-read confidence that separates "couldn't read it" from "read it fine, don't stock it" | ✅ Built · deterministic | `extraction_confidence()` blends header parse quality with the **better of** catalog-link ratio and a clearly-parsed-drug-row ratio (a row with a name plus a strength / dose pattern / directions / duration / quantity), discounted against linked rows. `vision_confidence()` instead uses the model's own `transcription_confidence`, scaled by the share of rows it marked legible. | Catalog linkage alone was the original measure, and it conflated two unrelated failures: a page that couldn't be read, and a page read perfectly whose drugs simply aren't stocked in Lebanon. Any foreign or out-of-catalog prescription scored 0 and was hidden from the patient behind `OCR_LOW_CONFIDENCE_THRESHOLD`. Loosening it is guarded on the other side — a model rating itself 1.0 while marking every row illegible, or reading no drug rows at all, still lands below the threshold, and letterhead fragments with no dose/quantity/match still score 0. Both directions are pinned by tests in `test_ocr_vision_structured.py`. |
| Patient-upload autofill: OCR the scan on upload, structure it into fields (patient, prescriber, date, and each medication with directions / duration / refills), show the patient that read **immutably** with a "flag for review" control, and let a pharmacist correct it inline before accept/reject | ✅ Built · deterministic default + optional non-Anthropic NLP seam | On `POST /api/shop/prescription-uploads/` the scan is OCR'd (`PRESCRIPTION_OCR_PROVIDER`) and passed through `apps/prescriptions/services/structured.py`, which runs the extractor named by `settings.PRESCRIPTION_NLP_PROVIDER`. Default `"regex"` reuses `apps/prescriptions/services/metadata.py` (prescriber / date / patient name+phone off labelled and title lines, EN+FR+AR) and `extraction.py` (catalog-matched drug lines) plus a regex pass for directions / duration / refills — deterministic, offline, no account. `"openai_compatible"` (+ `PRESCRIPTION_NLP_BASE_URL`/`_API_KEY`/`_MODEL`) sends the OCR **text** (not the image) to an OpenAI-compatible chat gateway — same bring-your-own-gateway seam as `apps.assistant.parsers.openrouter` / `apps.analytics.providers`, **deliberately not the Anthropic path** — for a real parse of messy layouts, falling back to the regex extractor on any failure so an upload is never lost. The model is boxed as a formatter: fixed output schema, keys allowlisted, lengths/list sizes bounded, told the OCR text is data not instructions. **Whichever extractor ran**, `structured.py` then reconciles every medication row against the catalog (`reconcile_medications` → `extraction.match_medicine` → the same `best_catalog_match()` search uses, with the dose-aware same-brand strength-variant preference): each row lands with `medicine_id` / `catalog_name` / `match_confidence` so a pharmacist acts on a real SKU, not a free-text string the model read. `_clean_structured_fields` re-derives that link from the corrected name/strength when a pharmacist edits a row, so fixing a misread drug name re-points it. `ocr_fields` is read-only to the patient (`ShopPrescriptionUploadSerializer`); `POST /api/shop/prescription-uploads/{id}/flag/` records "the OCR got this wrong" + a note. Pharmacists work the queue at `PharmacyPrescriptionUploadViewSet` (`/api/pharmacy/prescription-uploads/`, new "Patient uploads" page): editing `ocr_fields` inline claims the upload for that pharmacy, then `accept` / `reject`. Nothing is dispensed off this — an accepted upload still goes through the normal pharmacist-driven sale flow. Same data-handling caveat as `PRESCRIPTION_OCR_PROVIDER` applies to whichever NLP gateway is configured. |
| Shared LLM fallback endpoint (`settings.LLM_FALLBACK_*`) | ✅ Built · cross-cutting | Every OpenAI-compatible chat/vision call in the codebase — the assistant intent parser and reply composer, the analytics narrative digest, prescription OCR (`openai_vision`) and prescription structuring (`openai_compatible`) — now routes through `apps/common/openai_chat.py`. Each caller tries its **own** gateway first (keys and budgets stay deliberately separate, per `apps.analytics.providers.base`), then one shared `LLM_FALLBACK_*` endpoint if that call fails, then its existing deterministic path (keyword parser / template reply / templated digest / regex extractor / empty `ocr_fields`). Intended use: a cheap/free key (e.g. a Google AI Studio Gemini key via `https://generativelanguage.googleapis.com/v1beta/openai`) as a cushion when one provider is rate-limited or down. It is a cushion, not capacity — a free tier has its own hard limits — and it is inert unless base URL, key and model are all set. `manage.py test` forces every one of these knobs off regardless of `.env`, so the suite never reaches a real endpoint. |
...
| Free-form natural-language digest / narrative trend summary | ✅ Built · optional (needs an AI provider) | `apps/analytics/services/narrative.py`, `GET /api/pharmacy/analytics/digest/`, "Digest" tab (first tab) in the pharmacy analytics UI (en/ar/fr). The model is a narrator only: it is handed the exact same JSON that `insights.generate_insights()`/`kpis.overview()`/`kpis.movement_classification()`/`kpis.demand_signals()` already computed and told to quote every figure verbatim, never estimate one, and never phrase a connection between two facts as causal (no "because"/"likely due to" — the original proposal's own example sentence is exactly what's disallowed). `settings.ANALYTICS_AI_PROVIDER` defaults to `"none"` (see `apps/analytics/providers/`, one adapter for any OpenAI-compatible `/chat/completions` gateway — OpenRouter, OpenCode Zen's compatible-model family, a local Ollama server, ...) - unconfigured, misconfigured, or a failed call all fall back to rendering the existing rule-based Smart Insights cards as plain paragraphs, marked `stale: true`, so the analytics screen never breaks or silently shows unlabelled non-AI text as if a model wrote it. Result is cached per pharmacy per payload-hash for 12h so a provider call only fires when the underlying numbers actually changed. Deliberately its own provider config, separate from `ASSISTANT_*` (`apps.assistant`'s chat widget) - that surface's own design never lets a model write user-facing prose at all, only pick an intent name; this one is a different, narrower risk (a model narrating a fixed, already-computed business-numbers payload), so it gets its own key/budget rather than sharing one. |
